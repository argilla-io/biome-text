...
export default {
      ...
      if (this.isRange) {
        let arr = this.currentIndex.map((index, i) => {
          if (i === this.focusSlider || this.fixed) {
            const val = fn(index);
            const range = this.fixed ? this.valueLimit[i] : [this.minimum, this.maximum];
  ...
  mounted() {
    ...
    if (typeof window === 'undefined' || typeof document === 'undefined') {
      this.printError('window or document is undefined, can not be initialization.');
    }
...
@import "@/assets/scss/components/range.scss";
</style>
