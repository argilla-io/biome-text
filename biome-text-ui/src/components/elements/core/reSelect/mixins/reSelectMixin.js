...
function filterGroups(search, label, values, groupLabel, customLabel) {
  return groups => groups.map((group) => {
    ...
    if (!group[values]) {
      console.warn(
        'Options passed to vue-select do not contain groups, despite the config.',
      );
      return [];
    }
...
export default {
  ...
  mounted() {
    ...
    if (!this.multiple && !this.clearOnSelect) {
      console.warn(
        '[Vue-select warn]: ClearOnSelect and Multiple props can’t be both set to false.',
      );
    }
    if (!this.multiple && this.max) {
      console.warn(
        '[Vue-select warn]: Max prop should not be used when prop Multiple equals false.',
      );
    }
    if (
      this.preselectFirst
