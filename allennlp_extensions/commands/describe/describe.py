...
from allennlp.modules import Seq2VecEncoder, TextFieldEmbedder, TokenEmbedder, Seq2SeqEncoder, SimilarityFunction
...
class DescribeRegistrable(Subcommand):
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser = parser.add_parser(name, description=description, help='Describe a registrable')
        subparser.add_argument('-t', '--type', type=str, default="",
                               help='The base class for the described name component (predictor, model, encoder or reader)')
        subparser.add_argument('-n', '--name', type=str, default="", help="the registered named")
...
def describe_registrable(args: argparse.Namespace) -> None:
    if not args.type:
        logging.warning('No type provide. Select one of [%s]',
                        ", ".join(["'%s'" % key for key, _ in __REGISTRABLE_MAP.items()]))
        exit(1)

    registrable_type = __REGISTRABLE_MAP[args.type]
    if not args.name:
        logging.warning('No name provided. Select an available name from [%s]', registrable_type.list_available())
    else:
