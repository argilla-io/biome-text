...
from allennlp.common.util import prepare_environment, import_submodules, sanitize
...
class Evaluate(Subcommand):
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser.add_argument('-p', '--predict',
                               type=bool,
                               default=False,
                               help='if active, include predictions on evaluation output')
...
# Change logic to include predictions
def evaluate(model: Model,
             instances: Iterable[Instance],
             data_iterator: DataIterator,
             cuda_device: int,
             with_prediction: bool = False) -> (Dict[str, Any], Iterable[Dict]):
    model.eval()
    ...
    prediction_info: List[Dict] = []

    for batch in generator_tqdm:
        model(**batch)

        outputs = model.forward_on_instances(batch, cuda_device)

        if with_prediction:
            prediction_info.extend(generate_prediction_info(instances, outputs))

        metrics = model.get_metrics()
        description = ', '.join(["%s: %.2f" % (name, value) for name, value in metrics.items()]) + " ||"
        generator_tqdm.set_description(description, refresh=False)

    return model.get_metrics(), sanitize(prediction_info)


def generate_prediction_info(instances, outputs) -> List[Dict]:
    result = []
    for output, instance in zip(outputs, instances):
        result.append({
            "input": instance,  # TODO convert instances to json
            "prediction": output,
        })
    return result


# Same as allenlp.commands.evaluate
def evaluate_from_args(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    metrics, _ = evaluate(model, instances, iterator, args.cuda_device, args.predict)
