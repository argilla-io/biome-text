...
class BiomePublishModel(Subcommand):
    def add_subparser(
        self, name: str, parser: argparse._SubParsersAction
    ) -> argparse.ArgumentParser:
        # pylint: disable=protected-access
        description = (
            """Uploads the configuration model (as tar.gz file) to a s3 bucket."""
        )
        subparser = parser.add_parser(
            name,
            description=description,
            help="Publish model binaries & config to a S3 bucket",
        )
        ...
        subparser.add_argument(
            "--s3-access-key-id", type=str, default=AWS_ACCESS_KEY_ID
        )
        subparser.add_argument(
            "--s3-secret-access-key", type=str, default=AWS_SECRET_ACCESS_KEY
        )
