...
from biome.text.modules.heads.task_prediction import SENTINEL
...
class ProfNer(TaskHead):
    def __init__(
        ...
        self._tag_layer = TimeDistributed(
            torch.nn.Linear(self.backbone.encoder.get_output_dim(), len(ner_tags))
        )
    ...
    def forward(
        ...
        embedded_tokens = self.backbone(subtokens, mask)
