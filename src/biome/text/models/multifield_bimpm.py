...
class MultifieldBiMpm(BiomeClassifierMixin, Model):
    ...
    def __init__(
        ...
        self.output_layer = Linear(self.classifier_output_dim, self.num_classes)
    ...
    def forward(
        ...
        """
        mask_record1 = util.get_text_field_mask(record1)
        mask_record2 = util.get_text_field_mask(record2)

        # embedding and encoding of record1
        embedded_record1 = self.dropout(self.text_field_embedder(record1))
        encoded_record1 = self.dropout(self.encoder(embedded_record1, mask_record1))

        # embedding and encoding of record2
        embedded_record2 = self.dropout(self.text_field_embedder(record2))
        encoded_record2 = self.dropout(self.encoder1(embedded_record2, mask_record2))
        ...
        def add_matching_result(matcher, encoded_premise, encoded_hypothesis):
            ...
            matching_result = matcher(
                encoded_premise, mask_record1, encoded_hypothesis, mask_record2
            )
        ...
        add_matching_result(
            ...
            encoded_record1[:, :, :half_hidden_size_1],
            encoded_record2[:, :, :half_hidden_size_1],
        )
        add_matching_result(
