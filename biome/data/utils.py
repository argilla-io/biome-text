...
def get_nested_property_from_data(data: Dict, property_key: str) -> Optional[Any]:
    ...
    if property_key in data:
        ...
        return get_nested_property_from_data(data.get(splitted_key[0]), sep.join(splitted_key[1:]))
...
def default_elasticsearch_sink(source_config: str, binary_path: str, es_batch_size: int):
    ...
    return dict(
        index=sanizite_index('prediction {} with {}'.format(file_name, model_name).lower()),
        type='docs',
...
def configure_dask_cluster(n_workers: int, worker_memory: int = 3e9):
    def create_dask_client(dask_cluster: str, cache_size: Optional[int], workers: int) -> Client:
        ...
        try:
            if dask_cluster == 'local':
                ...
                dask.config.set({
                    'distributed.worker.memory': dict(target=0.95, spill=False, pause=False, terminate=False)
                })
                cluster = LocalCluster(n_workers=0, threads_per_worker=1, memory_limit=worker_memory)
    ...
    if dask_cluster:
        dask_client = create_dask_client(dask_cluster, dask_cache_size, workers=n_workers)
    else:
