...
class BiomePredict(Subcommand):
    ...
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser.add_argument('--workers', type=int, default=1, help='Workers for dask local cluster')
...
def __predictor_from_args(archive_file: str, cuda_device: int, include_package: List[str] = []) -> Predictor:
...
def _predict_deprecated(args: argparse.Namespace) -> None:
    ...
    def make_predict(batch: List[Dict[str, Any]], predictor: Predictor, sink_config: Dict[str, Any]):
...
def predict(
        binary: str,
        from_source: str,
        to_sink: Optional[str] = None,
        workers: int = 1,
        worker_mem: int = 2e9,
        batch_size: int = 1000,
        cuda_device: int = -1) -> None:
    def predict_partition(partition: Iterable) -> Dict[str, Any]:
        predictor = __predictor_from_args(archive_file=to_local_archive(binary), cuda_device=cuda_device)
        return predictor.predict_batch_json(partition)

    to_sink = default_elasticsearch_sink(from_source, binary, batch_size) if to_sink is None else to_sink

    source_config = read_datasource_configuration(from_source)
    sink_config = read_datasource_configuration(to_sink)

    configure_dask_cluster(workers, worker_mem)

    test_dataset = read_dataset(source_config, include_source=True).persist()
    predicted_dataset = test_dataset \
        .repartition(npartitions=workers) \
